...
["Senko-San"] = {
	...
	if load.IsLoaded == false then
		...
		if loadupClosed == false then
                ...
                GUIObject.Screen.Enabled = true
--// =========================
--//  MODERN UI PATCH V7.1 - FIXED VERSION
--//  - Fixed Tone button not working
--//  - Fixed tab size increasing on repeated clicks
...
do
    task.defer(function()
        ...
        local function translateText(s)
            ...
            for k, v in pairs(PrefixMap) do
                if s:sub(1, #k) == k then
                    ...
                    -- Translate tail if in DictExact
        ...
        local function buildMainUI()
            ...
            ToneBtn.Position = UDim2.new(0, 210, 0.5, 0)
            ToneBtn.Size = UDim2.new(0, 140, 0, 36)
            ToneBtn.ZIndex = 61
            ...
            -- Also hook original ToneButton if exists
            pcall(function()
            ...
            Search.Position = UDim2.new(0.65, 0, 0.5, 0)
            Search.Size = UDim2.new(0.32, 0, 0, 40)
            Search.ZIndex = 61
            mkCorner(Search, 12)
            ...
            BtnClose.Size = UDim2.new(0, 48, 0, 40)
            BtnClose.ZIndex = 61
            ...
            BtnMin.Size = UDim2.new(0, 48, 0, 40)
            BtnMin.ZIndex = 61
            mkCorner(BtnMin, 12)
            ...
            -- KEEP ButtonFrame visible but hidden off-screen so ToneButton still works
            if GUIObject.ButtonFrame and GUIObject.ButtonFrame:IsA("Frame") then
                GUIObject.ButtonFrame.Position = UDim2.new(10, 0, 10, 0) -- Move off-screen
                GUIObject.ButtonFrame.Visible = true -- Keep visible for event handling
            end
            ...
            local ScrollWrap = {}
            local InitialCanvasSize = {} -- Store initial canvas sizes
            ...
            local function makeScrollablePage(legacyPage)
                ...
                local function refresh()
                    task.defer(function()
                        ...
                        -- Only set initial size once, or if content actually grows
            ...
            local function setPage(rec, btn)
                -- ===== FIX: Prevent re-processing same tab =====
                ...
                -- Only refresh canvas size for scrollable pages when first switching to them
                ...
                if lname and ScrollWrap[lname] then
                    task.defer(function()
                        ...
                        if wrap and rec.legacy then
                            task.wait()
                            -- Use stored initial size if available
            ...
            local function showUI()
                ...
                Main.Active = false
                pcall(function() Main.Draggable = false end)
                -- Update tone button text when showing UI
                ToneBtn.Text = getCurrentToneText()
            end
            ...
            local function reorderClothesForBundle(bundleName)
                ...
                pcall(function()
                    ...
                    if not Bundle or not bundleName or not Bundle[bundleName] then
                        for _, it in ipairs(GUIObject.ClothesButtonFrame:GetChildren()) do
                            if it:IsA("Frame") then it.LayoutOrder = 0 end
                        end
                        return
                    end
                    local b = Bundle[bundleName]
                    local list = b and b.Clothing
            ...
            -- Update ToneBtn periodically to stay in sync
            task.spawn(function()
