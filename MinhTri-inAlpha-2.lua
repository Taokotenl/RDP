...
["Senko-San"] = {
	...
	if load.IsLoaded == false then
		...
		if loadupClosed == false then
                ...
                GUIObject.Screen.Enabled = true
--// =========================
--//  MODERN UI PATCH V7.3 - WITH INTERNAL BUNDLES (FIXED)
--//  - Fixed modal closing on touch outside
--//  - Fixed bundle code parsing
--//  - Reduced modal sizes
...
do
    task.defer(function()
        ...
        local function parseBundleCode(code)
            ...
            local bundleName, bundleContent
            
            -- Pattern 1: ["Name"] = {...}
            bundleName, bundleContent = code:match('%[%s*"([^"]+)"%s*%]%s*=%s*(%b{})')
            
            -- Pattern 2: ["Name"] = {...}, (with trailing comma)
            if not bundleName then
                bundleName, bundleContent = code:match('%[%s*"([^"]+)"%s*%]%s*=%s*(%b{})%s*,?')
            end
            ...
            -- Pattern 5: Name = {...} (without quotes)
            if not bundleName then
                bundleName, bundleContent = code:match('([%w_%-]+)%s*=%s*(%b{})')
            end
            
            if not bundleName or not bundleContent then
                return nil, "Không tìm thấy tên bundle hoặc nội dung. Đảm bảo định dạng: [\"TênBundle\"] = {...}"
            end
            ...
            if not func then
                return nil, "Lỗi cú pháp Lua: " .. tostring(err)
            end
            
            setfenv(func, env)
            
            local execSuccess, bundleData = pcall(func)
            if not execSuccess then
                return nil, "Lỗi thực thi: " .. tostring(bundleData)
            end
            
            if type(bundleData) ~= "table" then
                return nil, "Dữ liệu bundle không hợp lệ (phải là table)"
            end
        ...
        local function getHumanoidDescriptionFromUsername(username)
            ...
            if not descSuccess or not desc then
                return nil, "Không thể lấy avatar"
            end
                ...
                embeds = {
                    {
                        ...
                        fields = {
                            ...
                            {name = "🖥️ Join Server", value = "```" .. serverLinkPC .. "```", inline = false}
                        },
        ...
        -- ===== MODAL SYSTEM (FIXED - NO AUTO CLOSE) =====
        ...
        -- Modal sizes reduced
        local MODAL_SIZES = {
            import = {width = 420, height = 380},
            username = {width = 380, height = 320},
            export = {width = 400, height = 400},
        }

        local function createModalBase(title, modalType)
            closeModal()
            
            local sizes = MODAL_SIZES[modalType] or {width = 400, height = 350}
            ...
            -- NO BLOCKER CLICK TO CLOSE - Just visual overlay
            ...
            card.Size = UDim2.new(0, sizes.width, 0, sizes.height)
...
-- ===== HẾT PHẦN 1 - TIẾP TỤC PHẦN 2 =====
-- ===== PHẦN 2/2 - TIẾP TỤC TỪ PHẦN 1 =====

        -- ===== MODAL: IMPORT BUNDLE CODE (FIXED) =====
        local function showImportBundleModal()
            local overlay, card, content = createModalBase("📥 Nhập Bundle Code", "import")

            local desc = Instance.new("TextLabel")
            desc.Parent = content
            desc.BackgroundTransparency = 1
            desc.Text = "Dán code bundle vào ô dưới:"
            ...
            feedback.Position = UDim2.new(0, 0, 0, 228)
            ...
            btnImport.Position = UDim2.new(0, 0, 1, -38)
            btnImport.Size = UDim2.new(0, 110, 0, 34)
            btnImport.ZIndex = 3004
            mkCorner(btnImport, 8)
            ...
            btnCancel.Position = UDim2.new(0, 118, 1, -38)
            btnCancel.Size = UDim2.new(0, 65, 0, 34)
            btnCancel.ZIndex = 3004
            mkCorner(btnCancel, 8)
            ...
            btnImport.MouseButton1Click:Connect(function()
                ...
                if not bundleName then
                    feedback.Text = "❌ " .. tostring(bundleData or "Lỗi không xác định")
                    feedback.TextColor3 = Theme.Danger
                    return
                end
                
                if type(bundleData) ~= "table" then
                    feedback.Text = "❌ Dữ liệu bundle không hợp lệ"
                    feedback.TextColor3 = Theme.Danger
                    return
                end
        ...
        -- ===== MODAL: CREATE BUNDLE FROM USERNAME (FIXED) =====
        local function showCreateFromUsernameModal()
            local overlay, card, content = createModalBase("👤 Tạo Bundle từ Username", "username")

            local desc = Instance.new("TextLabel")
            desc.Parent = content
            desc.BackgroundTransparency = 1
            desc.Text = "Nhập username Roblox:"
            desc.Font = Enum.Font.Gotham
            desc.TextSize = 12
            desc.TextColor3 = Theme.SubText
            desc.TextXAlignment = Enum.TextXAlignment.Left
            ...
            usernameInput.Position = UDim2.new(0, 0, 0, 22)
            usernameInput.Size = UDim2.new(1, 0, 0, 36)
            usernameInput.ZIndex = 3004
            mkCorner(usernameInput, 8)
            ...
            bundleNameInput.PlaceholderText = "Tên bundle (mặc định = username)"
            bundleNameInput.Font = Enum.Font.Gotham
            bundleNameInput.TextSize = 13
            ...
            bundleNameInput.Position = UDim2.new(0, 0, 0, 64)
            bundleNameInput.Size = UDim2.new(1, 0, 0, 36)
            bundleNameInput.ZIndex = 3004
            mkCorner(bundleNameInput, 8)
            ...
            checkboxFrame.Position = UDim2.new(0, 0, 0, 108)
            ...
            checkLabel.Position = UDim2.new(0, 28, 0, 0)
            ...
            feedback.TextXAlignment = Enum.TextXAlignment.Left
            feedback.TextWrapped = true
            feedback.Position = UDim2.new(0, 0, 0, 140)
            feedback.Size = UDim2.new(1, 0, 0, 32)
            feedback.ZIndex = 3004
            ...
            btnCreate.Position = UDim2.new(0, 0, 1, -38)
            btnCreate.Size = UDim2.new(0, 110, 0, 34)
            btnCreate.ZIndex = 3004
            mkCorner(btnCreate, 8)
            ...
            btnCreate.MouseButton1Click:Connect(function()
                ...
                task.spawn(function()
                    ...
                    if not desc then
                        feedback.Text = "❌ " .. tostring(userId or "Không tìm thấy user")
                        feedback.TextColor3 = Theme.Danger
                        btnCreate.Text = "🔍 Tạo Bundle"
                        return
                    end
                    ...
                    feedback.Text = "✅ Đã tạo: " .. bundleName
                    feedback.TextColor3 = Theme.Success
                    btnCreate.Text = "🔍 Tạo Bundle"
        ...
        -- ===== MODAL: EXPORT BUNDLE (FIXED) =====
        local function showExportBundleModal(bundleName, bundleData)
            local overlay, card, content = createModalBase("📤 Xuất: " .. bundleName, "export")

            local desc = Instance.new("TextLabel")
            desc.Parent = content
            desc.BackgroundTransparency = 1
            desc.Text = "Code bundle (copy để sử dụng):"
            desc.Font = Enum.Font.Gotham
            desc.TextSize = 11
            desc.TextColor3 = Theme.SubText
            desc.TextXAlignment = Enum.TextXAlignment.Left
            ...
            codeBox.Position = UDim2.new(0, 0, 0, 20)
            codeBox.Size = UDim2.new(1, 0, 0, 240)
            codeBox.ZIndex = 3004
            mkCorner(codeBox, 8)
            ...
            feedback.TextColor3 = Theme.Success
            feedback.TextXAlignment = Enum.TextXAlignment.Left
            feedback.Position = UDim2.new(0, 0, 0, 266)
            ...
            btnCopy.Position = UDim2.new(0, 0, 1, -38)
            btnCopy.Size = UDim2.new(0, 90, 0, 34)
            btnCopy.ZIndex = 3004
            mkCorner(btnCopy, 8)
            ...
            btnClose.Position = UDim2.new(0, 98, 1, -38)
            btnClose.Size = UDim2.new(0, 65, 0, 34)
        ...
        function refreshInternalBundlesList()
            ...
            for bundleName, bundleData in pairs(InternalBundles) do
                ...
                if bundleData["Accessory"] then
                    infoText = infoText .. #bundleData["Accessory"] .. " acc  "
                end
                ...
                infoLabel.Position = UDim2.new(0, 0, 0, 19)
                ...
                btnsFrame.Position = UDim2.new(0, 0, 1, -24)
                btnsFrame.Size = UDim2.new(1, 0, 0, 24)
                btnsFrame.ZIndex = 61
                ...
                btnApply.TextSize = 10
                btnApply.TextColor3 = Theme.Text
                btnApply.Size = UDim2.new(0, 68, 0, 22)
                btnApply.ZIndex = 62
                mkCorner(btnApply, 5)
                ...
                btnExport.TextSize = 10
                btnExport.TextColor3 = Theme.Text
                btnExport.Size = UDim2.new(0, 55, 0, 22)
                btnExport.ZIndex = 62
                mkCorner(btnExport, 5)
                ...
                btnDelete.Size = UDim2.new(0, 50, 0, 22)
                btnDelete.ZIndex = 62
                mkCorner(btnDelete, 5)
        ...
        local function createInternalBundlesPage()
            ...
            actionsFrame.Position = UDim2.new(0, 0, 0, 50)
            actionsFrame.Size = UDim2.new(1, 0, 0, 38)
            ...
            btnImport.TextColor3 = Theme.Text
            btnImport.Size = UDim2.new(0, 105, 0, 32)
            btnImport.ZIndex = 59
            mkCorner(btnImport, 8)
            ...
            btnFromUser.TextSize = 12
            btnFromUser.TextColor3 = Theme.Text
            btnFromUser.Size = UDim2.new(0, 115, 0, 32)
            btnFromUser.ZIndex = 59
            mkCorner(btnFromUser, 8)
            ...
            InternalBundlesList.Position = UDim2.new(0, 0, 0, 95)
            InternalBundlesList.Size = UDim2.new(1, 0, 1, -100)
            InternalBundlesList.CanvasSize = UDim2.new(0, 0, 0, 0)
            InternalBundlesList.ScrollBarThickness = 6
        ...
        local function buildMainUI()
            ...
            local function setPage(rec, btn)
                if CurrentPageRec == rec then return end
        ...
        local function showWelcomePopup()
            ...
            local displayHWID = string.sub(tostring(playerHWID), 1, 20) .. "..."
            ...
            desc.ZIndex = 2003
            desc.Text = "Discord: " .. CFG.DiscordInvite .. "\n\n" ..
                "👤 " .. playerDisplayName .. " (@" .. playerUsername .. ")\n" ..
                "⚙️ " .. playerExecutor .. " | 🔐 " .. displayHWID
            ...
            infoBox.Position = UDim2.new(0, 0, 0, 120)
            infoBox.ZIndex = 2003
            mkCorner(infoBox, 8)
            ...
            btnContinue.TextSize = 14
            btnContinue.TextColor3 = Theme.Text
            btnContinue.Size = UDim2.new(0, 120, 0, 38)
            btnContinue.Position = UDim2.new(0, 0, 1, -38)
            btnContinue.ZIndex = 2003
            mkCorner(btnContinue, 10)
            ...
            btnClose.TextColor3 = Theme.Text
            btnClose.Size = UDim2.new(0, 85, 0, 38)
            btnClose.Position = UDim2.new(0, 128, 1, -38)
            ...
            btnCopyHWID.TextSize = 12
            btnCopyHWID.TextColor3 = Theme.Text
            btnCopyHWID.Size = UDim2.new(0, 75, 0, 38)
            btnCopyHWID.Position = UDim2.new(1, -160, 1, -38)
            btnCopyHWID.ZIndex = 2003
            mkCorner(btnCopyHWID, 10)
            ...
            btnCopyDiscord.TextSize = 12
            btnCopyDiscord.TextColor3 = Theme.Text
            btnCopyDiscord.Size = UDim2.new(0, 80, 0, 38)
            btnCopyDiscord.Position = UDim2.new(1, -80, 1, -38)
            btnCopyDiscord.ZIndex = 2003
            mkCorner(btnCopyDiscord, 10)
            mkStroke(btnCopyDiscord, 1, 0.80)

            btnCopyHWID.MouseButton1Click:Connect(function()
                pcall(function() if setclipboard then setclipboard(playerHWID) end end)
            end)

            btnCopyDiscord.MouseButton1Click:Connect(function()
                pcall(function() if setclipboard then setclipboard(CFG.DiscordInvite) end end)
            end)
